...
def hands(frame, a, checkArr):
    # טשטוש תמונות לקצוות חלקים יותר, רך
    blur = cv2.GaussianBlur(frame, (5, 5), 0)
    #ממיר את התמונה לאפור
    gray = cv2.cvtColor(blur, cv2.COLOR_BGR2GRAY)  # BGR -> GRAY CONVERSION
    #מסנן את כל מה שקטן מ70 ל255
    ...
    for (x, y, w, h) in hand:  # MARKING THE DETECTED ROI
        ...
        cv2.putText(frame, "Identify a hands", (10, 180),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (122, 122, 0), False)
        cv2.imwrite(r'C:\Users\henri\Documents\project\python-19-06\processedVideos\images\hand.jpg', frame)
        # img=cv2.imread(frame,1)
        # cv2.imwrite(os.path.join('processedVideos/images/ '+'waka.jpg'), img)

        checkArr[3] = True
        # עדכון במערך שזיהה ידיים
        a[3] += 1
...
def detect(gray, frame, a):
    # בדיקה אם לא זוהה כדי שבפעם הבא יהיה מול המצלמה
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.imwrite(r'C:\Users\henri\Documents\project\python-19-06\processedVideos\images\face.jpg', frame)

        #  בתוך האזור של הפנים: טווח התמונה וטווח האפור
        ...
        for (x_eye, y_eye, w_eye, h_eye) in eye:
            ...
            cv2.putText(frame, "The eyes are focused on the camera", (10, 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 180, 60), False)
            cv2.imwrite(r'C:\Users\henri\Documents\project\python-19-06\processedVideos\images\eye.jpg', frame)
            checkArr[1] = True
            a[1] += 1
        ...
        for (sx, sy, sw, sh) in smiles:
            ...
            cv2.putText(frame, "Identify a smile", (10, 130),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), False)
            cv2.imwrite(r'C:\Users\henri\Documents\project\python-19-06\processedVideos\images\smile.jpg', frame)
            checkArr[2] = True
            a[2] += 1
    if checkArr[0] == False and checkArr[1] == False and checkArr[2] == False:
        cv2.putText(frame, "your face are not in front the camera", (10, 300),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), False)
        cv2.imwrite(r'C:\Users\henri\Documents\project\python-19-06\processedVideos\images\notFace.jpg', frame)
